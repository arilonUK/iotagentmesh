...
export async function forwardToEndpointsHandler(
  ...
  try {
    // Build the target URL - forward to the api-endpoints function
    const baseUrl = Deno.env.get('SUPABASE_URL');
    if (!baseUrl) {
      throw new Error('SUPABASE_URL environment variable not set');
    }
    
    // Forward the path directly to api-endpoints
    const targetUrl = `${baseUrl}/functions/v1/api-endpoints${path}`;
